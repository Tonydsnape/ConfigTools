...
将当前目录中的 Excel 配置导出为客户端或服务端使用的 JSON 与 MessagePack 文件。

## 使用

- 双击 `ExportClient.bat` 导出客户端配置。
...
输出文件位于 `Client/json`、`Client/bytes`、`Server/json`、`Server/bytes`。导出成功时会整体替换目标端的 `json` 与 `bytes` 目录；校验失败不会修改已有产物。
...
`base` 表第 3 行必须为 `KeyName / Type / Value`，数据从第 4 行开始，`Key` 必须为 1。Value 根据 Excel 原生类型读取；以 `{` 或 `[` 开头的文本按 JSON5 对象或数组解析。
...
任意结构、类型或数据错误都会输出工作簿、sheet、行号和单元格位置，并终止整个目标端的导出。独立校验工具会尽量一次收集全部错误，并将结果保存到 `Tools/ValidationReport.txt`；检测过程不会修改 Client 或 Server 产物。
